...
import type { EditOp } from '@/lib/schedule-types';
import { cn } from '@/lib/utils';

const SUGGESTIONS = [
  'Change the accent color to coral pink',
  'Swap all Reshma classes to Anjali',
  'Add a 6:00 AM HIIT class on Friday',
  'Make the date range "July 7th - July 13th 2026"',
  'Change the beginner band color to mint green',
  'Remove the 7:30 PM class on Monday',
];

export function ChatPanel() {
  const {
    messages, isChatting, chatError,
    current, pushMessage, applyOps, setChatting, setChatError,
  } = useScheduleStore();
  ...
  const send = useCallback(async (text: string) => {
    ...
    if (!trimmed || !current || isChatting) return;
    setInput('');
    ...
    try {
      ...
      if (Array.isArray(data.operations) && data.operations.length > 0) {
        applyOps(data.operations as EditOp[], data.summary || `${data.operations.length} edit(s)`);
      }
    } catch (err: unknown) {
  ...
  }, [current, isChatting, messages, pushMessage, applyOps, setChatting, setChatError]);
    ...
    <div className="flex flex-col h-full bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
      ...
      >
        <input
          ...
          placeholder={current ? "Ask to edit anything…" : "Upload a schedule first…"}
          disabled={!current || isChatting}
        ...
        <button
          type="submit"
          disabled={!current || isChatting || !input.trim()}
